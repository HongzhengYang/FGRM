...
from methods.losses import dce_evidence_u_loss,\
    policy_gradient_loss_ece_calibration, nll_evidence_loss
import wandb
...
class BaseMethod:
    ...
    def __init__(self, configs):
        ...
        self.best_model_path, self.final_model_path, self.figures_path, \
    ...
    def train_FGRM(self, test_video_loader, val_loader, test_loader):
        ...
        for _, epoch in zip(np.linspace(0.01, 0.99, num=self.fgrm_epochs), range(self.fgrm_epochs)):
            # Train
            train_loss, all_head_losses = self.train_FGRM_epoch(self.model, val_loader, self.optimizer)
    ...
    def diag_fisher(self, model, train_loader):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            total_loss = policy_gradient_loss_ece_calibration(self.configs.edl_uncertainty, edl_u, target,
    ...
    def train_FGRM_epoch(self, model, train_loader, optimizer):
        ...
        self.precision_matrices = self.diag_fisher(deepcopy(model), train_loader)
    ...
    def train_EDL(self, train_loader, val_loader):
        ...
        for alpha, epoch in zip(np.linspace(0.01, 0.99, num=self.num_epochs), range(self.num_epochs)):
            ...
            print(f'Epoch: {best_epoch}  Best val dice : {best_val_dice:.6f} '
                  f'Best val ece : {best_val_ece:.6f} Best val mi : {best_val_mi:.6f}.')
            if self.wandb_active:
            ...
            torch.save(self.model.state_dict(), self.models_path / str(epoch))
    ...
    def folders(self):
        ...
        return best_model_path, final_model_path, figures_path, metrics_out_path, results_csv_file, models_path
