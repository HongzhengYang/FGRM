...
def policy_gradient_loss_mi_sample(edl_uncertainty, edl_u, p, alpha, evidence, batch_idx):
    device = get_device()
    batch_size = evidence.shape[0]
    num_classes = evidence.shape[1]

    expected_prob = torch.nn.functional.normalize(alpha, p=1, dim=1)
    mi = calc_mi(evidence.detach().cpu().numpy(), p.detach().cpu().numpy(), sample_wise=True)
    log_probs = torch.zeros(batch_size)
    for i in range(batch_size):
        m = Dirichlet(alpha[i].permute(1, 2, 0).view(-1, num_classes))
        log_probs[i] = torch.sum(m.rsample()) \
                       / (target_img_size * target_img_size)
    log_probs = log_probs.view(batch_size, -1)
    log_probs = torch.sum(log_probs, dim=1)
    loss_ece = - log_probs.to(device) * mi.to(device)
    return loss_ece


def policy_gradient_loss_ece_per_pixel(edl_uncertainty, edl_u, p, alpha, evidence, batch_idx):
    device = get_device()
    batch_size = evidence.shape[0]
    num_classes = evidence.shape[1]

    alpha = alpha.view(alpha.size(0), alpha.size(1), -1)  # N,C,H,W => N,C,H*W
    alpha = alpha.transpose(1, 2)  # N,C,H*W => N,H*W,C
    alpha = alpha.contiguous().view(-1, alpha.size(2))  # N,H*W,C => N*H*W,C
    target = p.view(-1, 1)
    target_one_hot = torch.FloatTensor(alpha.shape).to(device)
    target_one_hot.zero_()
    target_one_hot.scatter_(1, target, 1)

    pt = torch.nn.functional.normalize(alpha, p=1, dim=1)
    squared_diff = (target_one_hot - pt) ** 2

    m = Dirichlet(alpha)
    probs = m.rsample()
    loss_ece = probs.to(device) * squared_diff.to(device)
    return loss_ece
...
def policy_gradient_loss_ece_calibration(edl_uncertainty, edl_u, p, alpha, evidence, batch_idx):
    ...
    return 0.01 * loss_ece - 0.1*prob_entropy


def policy_gradient_loss_ece_alpha_calibration(edl_uncertainty, edl_u, p, alpha, evidence, batch_idx):
    device = get_device()
    batch_size = evidence.shape[0]
    num_classes = evidence.shape[1]
    ...
    alpha_zero = torch.sum(alpha, dim=1, keepdim=True)
    alpha_zero = torch.log(alpha_zero)
    alpha_zero = torch.square(alpha_zero)
    target = p.view(-1, 1)
    target_one_hot = torch.FloatTensor(alpha.shape).to(device)
    target_one_hot.zero_()
    target_one_hot.scatter_(1, target, 1)
    m = Dirichlet(alpha)
    eps = 1e-5
    target_one_hot = torch.nn.functional.softmax(target_one_hot/0.1, dim=1)
    log_density = m.log_prob(target_one_hot)

    log_density = torch.mean(log_density)
    alpha_zero = torch.mean(alpha_zero)
    return -log_density*0.01 + 0.0005*alpha_zero



def policy_gradient_loss_ece_density(edl_uncertainty, edl_u, p, alpha, evidence, batch_idx):
    device = get_device()
    batch_size = evidence.shape[0]
    num_classes = evidence.shape[1]
